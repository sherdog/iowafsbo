...
window.onload = function() {
	swfu = new SWFUpload({
		upload_script : "/manage/uploadadd.php?action=upload&code=<?=$form_code?>&id=<?=$form_id?>&type=4&session=<?=$session?>",
...
switch($form_action) {
	...
	case "delete":
		...
		break;
	case "upload": ?>
<a href='upload-slideshow2.php?id=<?=$form_id?>&code=<?=$form_code?>'>Done uploading</a><br>
...
<?
		break;
	default:
		echo "<a href='upload-slideshow2.php?action=upload&id=$form_id&code=$form_code'>Upload Slideshow</a><br>\n";
		showAttachments($form_code, 4, $form_id);
}
