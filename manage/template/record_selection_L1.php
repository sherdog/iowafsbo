...
define('defaultParams', "?referer=$form_referer&((L1id))=$form_((L1id))");
if(isset($form_referer) && $form_referer != ""){
	if($form_referer == "products.php")
		define('return_php', $form_referer."?action=edit&((L1id))=$form_((L1id))&id=$form_L2id");
}
else
...
function showRecords($((L1id))){
	global $((L1_table));
	$((L1_table))_title = $((L1_table))["((L1_table))_title"];
	$result = dbRows(defaultTable, "%((sortby))", 'ASC', "WHERE ((table))_((L1_table))=$((L1id))");
...
switch($form_action) {
	...
	case "add":
		$id = 0;
		showForm($form_((L1id)), 0);
		break;
	case "edit":
		showForm($form_((L1id)), $form_id);
		break;
	case "delete":
		askDelete($form_((L1id)), $form_id);
		break;
	case "list1":
		showRecords($form_((L1id)));
		break;
	default:
		selectProductCategoryRecords(this_php, "?action=list1");
}
///////////////////////////////////////////////////////////////////////////////////////////////////////////
